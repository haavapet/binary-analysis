...
def create_graphs(instrs, call_candidate, ret_candidate, pc_inc_per_instr, pc_offset, step):
    ...
    function_prologues = []
    for e in instrs:
        e.call_instruction = None # remove call pointers from previous iterations of this function call
        if e.call_opcode == call_candidate and (e.call_operand - step * pc_inc_per_instr - pc_offset) % pc_inc_per_instr == 0:
            ...
            if address < len(instrs) and instrs[address].ret_opcode == ret_candidate:
                ...
                e.call_instruction = instrs[address + step] # we link the call instruction to the function prologue it calls
    ...
    for i, (fb, start, end) in enumerate([(instrs[start:end], start, end) for start, end in zip(function_prologues, function_prologues[1:])]):
        for instr in fb:
    ...
    for fb in function_blocks:
        ...
        for instr in fb.instructions:
            if instr.call_instruction != None:
                called_functions += [instr.call_instruction.function_block]
        fb.called_function_blocks = called_functions

    return [{"f_id": e.id, "calls_f_id": list(set(e.called_function_blocks)), "start": e.start, "end": e.end} for e in function_blocks]
